...
(** max size for the top bar of the multi-line task bar *)
let top_bar_size_default = 100

(** do not attempt to draw the top bar of the multi-line task bar unless it can be at least this big *)
...
let move_cursor_up n = P.sprintf "\027[%iA" n
...
let draw_top_bar fmt ~term_width ~total ~finished ~elapsed =
  ...
  ( if progress_bar_size < min_acceptable_progress_bar then
    let s = P.sprintf "%d/%s %s" finished tasks_total_string elapsed_string in
...
let set_tasks_total task_bar n =
  match task_bar with MultiLine t -> t.tasks_total <- n | NonInteractive | Quiet -> ()


let set_remaining_tasks task_bar n =
  match task_bar with
  | MultiLine t ->
      t.tasks_done <- t.tasks_total - n
  | NonInteractive | Quiet ->
      ()
